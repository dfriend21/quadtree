...
#' 
#' Since a quadtree may contain \code{NA} values, \code{transform_fun} must
#' be able to handle \code{NA}s without throwing an error. For example, if
#' \code{transform_fun} contains some control statement such as \code{if(x <
...
#' to maintain a version of the original quadtree, use \code{\link{qt_copy}} 
...
#' 
#' # create a quadtree
#' qt1 = qt_create(rast, split_threshold = .1)
#' 
#' # copy the quadtree so that we have a copy of the original (since using 
#' #'qt_transform' modifies the quadtree object)
#' qt2 = qt_copy(qt1)
#' qt3 = qt_copy(qt1)
#' qt4 = qt_copy(qt1)
#' 
#' qt_transform(qt2, function(x) 1-x)
#' qt_transform(qt3, function(x) x^3 ) 
#' qt_transform(qt4, function(x){
