...
#' 
#' Note that typically the quadtree isn't particularly space-efficient- it's not
#' uncommon for a quadtree file to be larger than the original raster file
#' (although, of course, this depends on how 'coarse' the quadtree is in 
#' relation to the original raster). This is likely because the quadtree has 
#' to store much more information about each cell (the x and y limits, its
#' value, pointers to its neighbors, among other things) while a raster can
#' store only the value since the coordinates of the cell can be determined
#' from the knowledge of the extent and the dimensions of the raster.
#' 
