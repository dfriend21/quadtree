...
test_that("qt_lcp_summary() runs without errors and produces expected output",{
  ...
  expect_error(qt_find_lcps(lcpf,limit_type="none"),NA)
  lcp_sum = expect_error(qt_lcp_summary(lcpf),NA)
...
test_that("qt_find_lcps() finds the same paths as in previous runs", {
  ...
  data(habitat)
  qt = qt_create(habitat,0,split_method="sd",min_cell_length=1000)
